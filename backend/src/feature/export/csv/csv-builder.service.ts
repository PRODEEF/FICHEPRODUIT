...
export class CsvBuilderService {
  /**
   * Produit le CSV avec une ligne d’en‑tête = noms des champs d’export, dans l’ordre donné.
   *
   * @param products Produits déjà mappés (une ligne par produit)
   * @param exportFields Ordre et libellés des colonnes (propriété `name`)
   */
  build(products: MappedProduct[], exportFields: ExportField[]): string {
