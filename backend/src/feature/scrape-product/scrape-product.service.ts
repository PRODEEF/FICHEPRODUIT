...
type ScrapeProductWarning = components['schemas']['ScrapeProductWarning'];
type ScrapeProductResponse = components['schemas']['ScrapeProductResponse'];

const FETCH_TIMEOUT_MS = 18_000;
const USER_AGENT =
  'Mozilla/5.0 (compatible; FicheProduit/1.0; +https://example.com/bot)';

@Injectable()
export class ScrapeProductService {
  scrapeProductPage(rawUrl: string): Promise<ScrapeProductResponse> {
  ...
  private async fetchAndExtract(url: string): Promise<ScrapeProductResponse> {
    const warnings: ScrapeProductWarning[] = [];
    let html = '';
    try {
  ...
  private extractJsonLdVariantsAndProperties(
    ...
    const add = (label: string, type: ProductTemplateField['type']) => {
  ...
  private extractPrestaShopVariantFields(
    ...
    $('ul.product-variants-list li .attribute-name, .variant-label').each(
      (_i, el) => {
        push($(el).text());
      },
    );
  ...
  private extractProductDetailFields(
    ...
    const fields: ProductTemplateField[] = [];
    const seen = new Set<string>();
    const add = (name: string, type: ProductTemplateField['type']) => {
    ...
    $('.product-tabs .nav-tabs .nav-link, ul.nav-tabs li .nav-link').each(
      (_i, a) => {
        ...
        if (/description|détail|caractéristiques|composition|livr|entretien|guide/i.test(
          t,
        )) {
    ...
    $(
      ...
      if (/description|détail|caract|composition|livr|entretien|guide|taille|couleur/i.test(
        t,
      )) {
    ...
    if (
      shortBlock.length &&
      shortBlock.text().replace(/\s+/g, ' ').trim().length > 20
    ) {
      add('Résumé / accroche', 'rich_text');
    }

    $(
      '.product-information .tab-pane, #product .tab-pane, .tabs .tab-pane',
    ).each((_i, pane) => {
      const $pane = $(pane);
  ...
  private extractFeatureFields(
    ...
    $('dl').each((_i, dl) => {
      const root = $(dl);
      if (
        root.closest('.product-features, #product-details, .product-information')
          .length === 0 &&
    ...
    if (labels.size === 0) {
      warnings.push({
        ...
        message:
          'No PrestaShop-style feature rows detected (table/dl heuristics)',
      });
      return [];
    }
  ...
  private mergeFields(rows: ProductTemplateField[]): ProductTemplateField[] {
    const seen = new Set<string>();
    const out: ProductTemplateField[] = [];
    for (const f of rows) {
