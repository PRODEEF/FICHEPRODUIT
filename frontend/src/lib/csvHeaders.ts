/**
 * Détection de la première ligne utile d’un CSV comme ligne d’en-têtes (export PrestaShop, etc.).
 */
...
/**
 * Ligne d’en-têtes + éventuels extraits de la première ligne de données (même séparateur).
 */
...
/**
 * Devine le type de champ gabarit produit à partir du libellé d’une colonne CSV PrestaShop (ou assimilé).
 */
export function inferProductTemplateFieldTypeFromCsvHeader(
  ...
  if (/^reference\b/i.test(raw) || /\battribute\s+reference\b/i.test(h)) {
    return 'reference';
  }
  if (
