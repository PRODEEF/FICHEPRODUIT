/**
 * Parse the first data row of a CSV (PrestaShop export) to column names.
 */
...
/**
 * Header row + optional first data row samples (same delimiter as header).
 */
export function parseCsvHeadersAndFirstDataRow(text: string): {
  ...
  const headers = parseDelimitedLine(headerLine, delim).filter(
    (h) => h.length > 0,
  )
  ...
  if (nonEmpty.length >= 2) {
    ...
    headers.forEach((h, i) => {
      ...
      sampleByHeader[h] =
        v.length > SAMPLE_PREVIEW_MAX
          ? `${v.slice(0, SAMPLE_PREVIEW_MAX)}…`
          : v
    })
  }
...
/**
 * Guess field type from a PrestaShop (or similar) CSV column title.
 */
export function inferProductTemplateFieldTypeFromCsvHeader(
  ...
  if (
    /^reference\b/i.test(raw) ||
    /\battribute\s+reference\b/i.test(h)
  ) {
    return 'reference'
  }
  if (
  ...
  if (h === 'description' || /^short\s+description\b/i.test(raw)) {
    return 'rich_text'
  }
