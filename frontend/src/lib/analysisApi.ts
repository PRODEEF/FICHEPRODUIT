...
import {
  normalizeSiteAnalysisJson,
  type SiteAnalysis,
} from './siteAnalysisPayload'
...
function normalizeProductListJson(raw: unknown): ProductListResponse {
  ...
  if (Array.isArray(productsRaw)) {
    for (const p of productsRaw) {
      ...
      products.push({
        ...
        currency:
          typeof o.currency === 'string' && o.currency ? o.currency : 'EUR',
        imageUrl:
  ...
  const total =
    typeof obj?.total === 'number' && !Number.isNaN(obj.total)
      ? obj.total
      : products.length
...
/** Starts a site analysis job (requires authenticated Supabase session). */
  ...
  if (!res.ok) {
    ...
    throw new Error(
      errorMessageFromBody(
        parsed,
        `Impossible de créer l’analyse (${res.status}).`,
      ),
    )
  }
...
/** Polls analysis status (requires authenticated Supabase session). */
export async function getSiteAnalysis(id: string): Promise<SiteAnalysis> {
  const res = await fetch(
    `${getApiBaseUrl()}/analyses/${encodeURIComponent(id)}`,
    {
      method: 'GET',
      headers: await analysesAuthHeaders(),
    },
  ...
  if (!normalized) {
    throw new Error(
      'Réponse serveur invalide : impossible de lire l’analyse (JSON inattendu).',
    )
  }
  return normalized
}

/** Lists analyses for the current user (requires authenticated Supabase session). */
export async function listSiteAnalyses(): Promise<SiteAnalysis[]> {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      errorMessageFromBody(
        parsed,
        `Impossible de lister les analyses (${res.status}).`,
      ),
    )
  }
...
/** Fetches products for a completed analysis (requires authenticated Supabase session). */
