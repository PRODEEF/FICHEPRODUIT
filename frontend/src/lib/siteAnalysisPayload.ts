...
function pickCms(
  obj: Record<string, unknown>,
): SiteAnalysis['cmsType'] | undefined {
...
/**
 * Maps various server JSON shapes (camelCase, snake_case, { data: {...} }) to SiteAnalysis.
 */
export function normalizeSiteAnalysisJson(raw: unknown): SiteAnalysis | null {
  ...
  const userId =
    pickUuid(obj, ['userId', 'user_id', 'UserId']) ??
    '00000000-0000-0000-0000-000000000000'
  const url = pickStr(obj, ['url', 'Url'])
  if (!id || !url) return null

  const createdAt =
    pickStr(obj, ['createdAt', 'created_at']) ?? new Date().toISOString()
  const updatedAt =
    pickStr(obj, ['updatedAt', 'updated_at']) ?? new Date().toISOString()
