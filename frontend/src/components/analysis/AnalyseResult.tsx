...
export function AnalyseResult({
  ...
  const brandsForChips = useMemo(() => {
    ...
    return brandOptions
  }, [analysis?.brandsList, brandOptions])
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((p: Product) => {
      ...
      if (filterSubCategory && p.subCategory !== filterSubCategory)
        return false
  ...
  const someFilteredSelected = filteredProducts.some((p) =>
  ...
  const importSelected = useCallback(() => {
    window.alert(
      `${selectedInViewCount} fiche(s) importée(s) avec succès !`,
    )
  }, [selectedInViewCount])
  ...
  const deleteSelected = useCallback(() => {
    ...
    if (
      !window.confirm(`Supprimer ${toRemove.length} fiche(s) de la vue ?`)
    ) {
      return
    }
    setRemovedIds((prev) => {
  ...
  return (
    <>
      ...
      {analysis.status === 'completed' ? (
        <>
          <div className="analyses-site-tag-wrap">
            <p className="analyses-site-tag" title={analysis.url}>
              🔗 {analysis.url} — {formatCmsLabel(analysis.cmsType)} —{' '}
              {brandCountDisplay} marques — {productCountDisplay} produits
            </p>
            {sellsLine ? (
              <p className="analyses-site-tag-sub">{sellsLine}</p>
            ) : null}
          </div>
          ...
          >
            ...
            >
                <h2
                  id="analyses-products-heading"
                  className="analyses-section-title"
                >
                  Produits proposés
                </h2>
                ...
                {brandsForChips.length > 0 ? (
                  <div
                    className="analyses-brand-chips"
                    aria-label="Marques principales"
                  >
                ...
                <div className="analyses-results-toolbar">
                  <p className="analyses-results-count">
                    <span id="total-count">{filteredProducts.length}</span>{' '}
                    produit(s) —{' '}
                    <span id="selected-count">{selectedInViewCount}</span>{' '}
                    sélectionné(s)
                  </p>
                        ...
                        {filteredProducts.map((p: Product) => (
                          ...
                          >
                            ...
                            <td className="analyses-table-cell-thumb">
                              {p.imageUrl ? (
                                <img
                                  className="analyses-product-thumb"
                                  src={p.imageUrl}
                                  alt=""
                                />
                              ) : (
                                <span className="analyses-thumb-placeholder">
                                  —
                                </span>
                              )}
                            </td>
                            <td>
                              {p.year ? (
                                <span className="analyses-year-tag">
                                  {p.year}
                                </span>
                              ) : (
                                '—'
                            ...
                            <td>{p.category ?? '—'}</td>
                            <td>{p.subCategory ?? '—'}</td>
                            <td className="analyses-table-cell-title">
                              {p.title}
                            </td>
                            <td className="analyses-table-cell-clamp">
                              {p.description ?? '—'}
                            </td>
        ...
        <p className="analyses-status">
          Statut&nbsp;: {STATUS_LABELS[analysis.status]}
        </p>
      ) : null}
    </>
  )
